...
const ChartAccount: React.FC = () => {
  ...
  const getAllOpeningsFilledPosition = (): Position[] => {
    return getAllOpeningsFilled().map(
      (opening) =>
        positions.find(
          (position) => position.id === opening.position_id,
        ) as Position,
    );
  };
  ...
  const getEmployeesBench = (): Employee[] => {
    return employees.filter(
      (employee) => employee.employee_status !== "On Hired",
    );
  };
  ...
  const getNonRepeatEmployeeProposedAction = (): string[] => {
    return Array.from(
      new Set(employees.map((employee) => employee.proposed_action)),
    );
  };

  const getSalaryBilling = (employee_id: number): number => {
    return employees.find((employee) => employee.id === employee_id)
      ?.salary as number;
  };
  ...
  const countAmountPersonsStatus = (): number[] => {
    ...
    return nonRepeatingStatus.map(
      (status) => persons.filter((person) => person.status === status).length,
    );
  };

  const countAmountEmployeesProposedAction = (): number[] => {
    const nonRepeatingProposedAction = getNonRepeatEmployeeProposedAction();
    return nonRepeatingProposedAction.map(
      (action) =>
        employees.filter((employee) => employee.proposed_action === action)
          .length,
    );
  };
  ...
  const countAmountEmployeesSalary = (): number[] => {
    ...
    for (let i = minSalary; i <= maxSalary; i += step) {
      labels.push(`${i}-${i + step}`);
      counts.push(
        salaryBilling.filter((salary) => salary >= i && salary <= i + step)
          .length,
      );
    }
    return counts;
  ...
  const chartBgColor = [
    ...
    "#FF564D",
    "#FF564D",
    determineColorIfPositive(
      getPositionsBillRateSum() - getTotalBenchSalary() - getTotalHiredSalary(),
    ),
  ];
  ...
  const chartData = [
    getPositionsBillRateSum(),
    getTotalHiredSalary(),
    getTotalBenchSalary(),
    getTotalBenchSalary() + getTotalHiredSalary(),
    getPositionsBillRateSum() - getTotalBenchSalary() - getTotalHiredSalary(),
  ];
    ...
    <div className="row r1">
      ...
      <div className="col-sm-6 c1">
        <div className="graph1-account">
          <h2 className="graph1-staffer-title">
          ...
          <ChartComponent
            ...
            data={chartData}
            labels={chartLabels}
            legendDisplay={legendDisplay2}
            legendposition={legendposition}
            labelcolor={labelcolor}
            axiscolor={axiscolor}
            bgcolor={chartBgColor}
            bdcolor={chartBdColor}
            bdwidth={chartBdWidths}
            indexAxis={indexAxis}
          />
        </div>
        ...
        <div className="graph4-staffer">
          <h2 className="graph1-staffer-title">
            Distribución de Acción Propuesta para Empleados
          </h2>
          <ChartComponent
            type={chartType3}
            data={chartData4}
            ...
            axiscolor={axiscolor}
            bgcolor={chartBgColor3}
            bdcolor={chartBdColor2}
            bdwidth={chartBdWidths2}
          />
        </div>
      </div>
      ...
      <div className="col-sm-6">
        ...
        <div className="graph3-staffer">
          ...
          <ChartComponent
            ...
            legendposition={legendposition}
            labelcolor={labelcolor2}
            axiscolor={axiscolor2}
            bgcolor={chartBgColor2}
            bdcolor={chartBdColor2}
            bdwidth={chartBdWidths3}
          />
        </div>
      </div>
    </div>
