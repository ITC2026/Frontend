...
const TablePipeline = (props: Props) => {
  ...
  const [salarioEsperado, setSalarioEsperado] = useState<{
    [key: number]: string;
  }>({});
  ...
  useEffect(() => {
    const fetchPersonData = async () => {
          ...
          if (props.categories[key]) {
            ...
            if (
              ...
              try {
                ...
                SalarioEsperadoMap[personId] =
                  personExpectedSalary?.toString() ||
                  "Error fetching expected salary";
              } catch (error) {
                console.error("Error fetching tech stack from ID:", error);
      ...
      setSalarioEsperado((prevSalario) => ({
        ...prevSalario,
        ...SalarioEsperadoMap,
      }));
    };
  ...
  return (
    <div>
      ...
      <Table striped bordered hover responsive bsPrefix="custom-table">
        ...
        <tbody>
          {filteredEntity.map((entity: Person, index: number) => (
            <tr key={index}>
              <td>{index + 1}</td>
              {Object.keys(props.categories).map(
                (key: string, columnIndex: number) => {
                  ...
                  if (props.categories[key].includes("Titulo de trabajo")) {
                    return <td key={columnIndex}>{jobTitles[personId]}</td>;
                  }
                  if (props.categories[key].includes("Tech Stack")) {
                    return <td key={columnIndex}>{techStacks[personId]}</td>;
                  }
                  if (props.categories[key].includes("Salario Esperado")) {
                    return (
                      <td key={columnIndex}>{salarioEsperado[personId]}</td>
                    );
                  } else {
